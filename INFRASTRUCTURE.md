...
- **CI/CD:** G1 `ruff check . && ruff format --check .` → G2 `mypy src/ --strict` → G3 `bicep build --file infra/main.bicep` → G4 `pytest tests/unit/ -v` (>= 85% coverage) → G5 e2e Triage API test.
...
- **Test:** `uv run pytest tests/unit/ -v` (add `pytest -q` for quick).
- **Lint:** `uv run ruff check . && uv run ruff format --check .`
- **Mock:** `aimock -c aimock/aimock.json -p 4010` for offline LLM/search/agentic mocks (see `aimock/README.md`).
